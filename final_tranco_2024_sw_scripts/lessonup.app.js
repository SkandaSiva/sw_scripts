...
const excludedDomainCaching = ['firestore.googleapis.com'];
...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    caches.open('version-cache').then(async (cache) => {
      return cache.put('sw-version', new Response(version));
    })
  );
});
...
self.addEventListener('fetch', (event) => {
  if (excludedDomainCaching.some((element) => event.request.url.includes(element))) {
    return;
  }
      ...
      return fetch(requestToFetch)
        .then((response) => {
          ...
          if (
            ...
            /chrome-extension/.test(event.request.url) ||
