...
const apiUrl_sport = '/templatedata/backupapi_sports.txt';

self.addEventListener('install', function (event) {
    event.waitUntil(
        caches.open(cacheName)
            .then(function (cache) {
                return cache.addAll([apiUrl, apiUrl_live, apiUrl_sport]);
            })
    );
});
...
function checkDomainAvailability() {
    return caches.open(cacheName)
        .then(function (cache) {
            const url = location.hostname.toLowerCase().indexOf('live') > -1 ? apiUrl_live : location.hostname.toLowerCase().indexOf('sports') > -1 ? apiUrl_sport : apiUrl;
