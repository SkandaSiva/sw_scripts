...
(function (self) {
  ...
  const RE = {
    method: /GET/i,
    static:
      /\.(?:png|jpe?g|css|js|json|gif|webm|webp|eot|svg|ttf|woff|woff2)(?:\?[a-zA-Z0-9-._~:/#\[\]@!$&'()*+,=]*)?$|(?:fonts\.googleapis\.com|gstatic\.com)/i,
    sockjs: /\/sockjs\//,
  };
  ...
  self.addEventListener('fetch', function (event) {
    ...
    if (event.request.headers.has('range')) {
      ...
      // if this is a range request. (206 code)
      return;
    }
  ...
  self.addEventListener('push', (event) => {
    ...
    if (serverData.postId) {
      ...
      const icon =
        serverData.userImage || 'https://cdn.ryfma.com/defaults/icons/favicon-196x196.png';
