...
const CACHE_VERSION = '2.1.0-beta6-v4';

// Never include these URLs in the SW cache.
const CACHE_EXCLUDE = ["admin\/.*","user\/.*","login\/.*","ticketing\/.*",".*\/api\/.*",".*\/ssks_api\/.*",".*\/concorde\/.*",".*\/main.umd.js",".*\/concorde.bundle.js",".*\/concorde-customer.bundle.js",".*\/kit.fontawesome.com\/.*"].map(function (r) {return new RegExp(r);});

// Cached pages. Add URLs using the 'Service Worker' tab of the Drupal UI.
let CACHE_URLS = [null,"https:\/\/lerocherdepalmer.fr\/sites\/lerocherdepalmer\/files\/pwa\/pwa-icon.png","https:\/\/lerocherdepalmer.fr\/sites\/lerocherdepalmer\/files\/pwa\/pwa-icon_192x192.png","https:\/\/lerocherdepalmer.fr\/sites\/lerocherdepalmer\/files\/pwa\/pwa-icon_144x144.png"];
