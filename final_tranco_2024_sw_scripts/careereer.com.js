...
self.addEventListener('push', function(event) {
  ...
  const options = {
    ...
    event.waitUntil(self.registration.showNotification(title, options).then(function() {
        ...
        let e = {site : 'en_careereer',u_s : utm_source};
...
self.addEventListener('notificationclick', function(event) {
  ...
  if(event.notification.data) { //seems that background notification shown by system sends data this way
    ...
    redirectUrl = 'https://www.careereer.com';
  }
  ...
  event.waitUntil(
    ...
    self.registration.pushManager.getSubscription()
      .then(function(subscription) {
         ...
         let e = {site : 'en_careereer',url : redirectUrl ,endpoint : end_point,utm_source : utm_source};
