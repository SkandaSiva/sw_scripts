"use strict";
const DEBUG = false;
const SW_CACHE_BUST = false;
const BASE_URL = "https://www.opel-insignia.su/";
const CACHED_ASSETS = ["https://www.opel-insignia.su/index.php?/offline/"];
const CACHE_NAME = 'invision-community-dbdfe5b5e01636711072';
const HTTP_CACHE_DURATION = 30;
const HTTP_CACHE_BUFFER = 60;
const OFFLINE_URL = "https://www.opel-insignia.su/index.php?/offline/";
const NOTIFICATION_ICON = null;
const DEFAULT_NOTIFICATION_TITLE = "У вас есть новое уведомление";
const DEFAULT_NOTIFICATION_BODY = "Чтобы прочитать это, вам нужно будет перейти на сайт. Нажмите здесь, чтобы войти в систему.";
/**
 * Invision Community
...
self.addEventListener("fetch", (e) => {
	...
	e.respondWith(
		new Promise((resolve, reject) => {
			...
			idbKeyval
				.getMany(["logged_in", "member_id"])
				.then(([logged_in, member_id]) => {
					...
					if (SW_CACHE_BUST && request.mode === "navigate" && request.method === "GET" && logged_in) {
					...
					if ( ( member_id === undefined || member_id === 0 ) && request.method === "POST") {
