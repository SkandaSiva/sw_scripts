...
var PushKaSw = function()
{
    ...
    this.mode       = 'debug';
    this.version    = 3;
    this.apiBaseUrl = self.location.host === 'localhost' || self.location.host === '127.0.0.1' ? 'http://'+self.location.host+'/api/' : 'https://burningapril.info/api/';
    ...
    function fetchAndShowMessage(subsId, subsEndpoint, serverId)
    {
        ...
        var url = sw.apiBaseUrl+'push/message?subsId='+encodeURIComponent(subsId)+(endpoint ? '&subsEndpoint='+encodeURIComponent(endpoint) : '')+(serverId ? '&srv='+encodeURIComponent(serverId) : '');
