...
const apiDomain = "https://pwa-api.dailyhunt.in"//"https://stage-pwabe.dailyhunt.in";//'https://pwa-api.dailyhunt.in';//https://pwaqa.dailyhunt.in';,'https://api-news.dailyhunt.in';
...
if (self) {
  ...
  isFunction(self.addEventListener) && self.addEventListener("fetch", event => {
    // console.log('///service wk fine', event.request);
  });
  ...
  */
  ...
  isFunction(self.addEventListener) && self.addEventListener('notificationclose', function (e) {
    ...
    self.logNotificationOpenEvent(notification, "close")
  ...
  isFunction(self.addEventListener) && self.addEventListener('notificationclick', function (event) {
    ...
    self.logNotificationOpenEvent(notification, "click");
    ...
    idbFunctions.set = function (key, val) {
      try {
        return dbPromise && dbPromise.then(db => {
          ...
          tx && tx.objectStore(storeName).get(key)
            .then((result) => {
              if (!result) {
              ...
              console.error("this is exception", e)
            })
  ...
  if (payload) {
    ...
    self.fetchCommonParams(idb, function (err, properties) {
      if (properties) {
        ...
        properties.site_mode = isBrowser && _.at(window, '__STATE.isDesktop') ? "desktop" : "mobile"
...
function logNotificationOpenEvent(notificationOpenPayload, action) {
  if (notificationOpenPayload) {
    ...
    self.fetchCommonParams(idb, function (err, properties) {
      if (properties) {
        ...
        properties.notification_action = action || "click";
        properties.site_mode = isBrowser && _.at(window, '__STATE.isDesktop') ? "desktop" : "mobile"
...
function sendGAAnalyticsEvent(category, action, label) {
  /* var trackingId = 'G-FFLMQZ6SGV';//'UA-112546024-1';//'UA-64780041-3'
  self.fetchCommonParams(idb, function (err, properties) {
    var payloadData = {
      v: 1,
      t: 'event',
