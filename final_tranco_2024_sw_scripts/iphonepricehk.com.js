...
// Remove old caches
    //console.log('current cache version: '+cacheVersion);
  event.waitUntil(
    (async () => {
      const keys = await caches.keys();
      return keys.map(async (cache) => {
        if(cache.indexOf(cacheVersion) < 0 ) {
          //console.log('Service Worker: Removing old cache: '+cache);
          return await caches.delete(cache);
        }else{
          //console.log('Service Worker: keep cache: '+cache);  
        }
      })
    })()
  )
...
registerRoute(
    ...
    ({ request, url }) => {
        //console.log(request.mode + ' vs ' + (request.mode === 'navigate' ? 't' : 'f') + ' vs' + url.href);
        return request.mode === 'navigate'
    },
    // Use a Network First caching strategy
    new NetworkFirst({
        // Put all cached files in a cache named 'pages'
        cacheName: 'pages'+cacheVersion,
        plugins: [
    ...
    ({ url, request }) => {
        ...
        */
        if (request.destination === 'image'
            && url.pathname.indexOf('timthumb.php') > 0
        ) {
            return true
        } else {
            return false
