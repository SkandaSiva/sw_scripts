    ...
    class Driver {
        ...
        handlePush(data) {
            return __awaiter$5(this, void 0, void 0, function* () {
                ...
                if (data.data && data.data.model && data.data.model === 'push_transacional') {
                    data.notification = {
                        data: data.data,
                ...
                if (data.data && data.data['gcm.notification.data']) {
                    let notificationData = JSON.parse(data.data['gcm.notification.data']);
                    data.notification.data = notificationData;
                }
                if (data.data && data.data['gcm.notification.picture']) {
                    data.notification.image = data.data['gcm.notification.picture'];
                }
                data.notification.badge = '/assets/icons/android-icon-192x192.png';
        ...
        handleClick(notification, action) {
            return __awaiter$5(this, void 0, void 0, function* () {
                ...
                yield this.handleModel(notification);
                yield this.broadcast({
        ...
        handleModel(notification) {
            let url = new URL('/', location).href;
            if (notification.data && notification.data.model) {
                if (notification.data.model === 'Produto') {
                    let id = notification.data.produto.id;
                    let slug = notification.data.produto.link;
                    url += "/produtos/detalhe/" + id + "/" + slug;
                } else if (notification.data.model === 'Colecao') {
                    let id = notification.data.colecao.id;
                    let slug = notification.data.colecao.slug;
                    url += "/produtos/colecoes/" + id + "/" + slug;
                } else if (notification.data.model === 'Oferta') {
                    let id = notification.data.oferta.id;
                    let slug = notification.data.oferta.nome;
                    let tipoOfertaId = notification.data.oferta.tipo_oferta_id;
                    if (tipoOfertaId == 4) { // COMBO DE PRODUTOS
                        url += "/produtos/combos/" + id;
                    } else {
                        url += "/produtos/oferta/" + id + "/" + slug;
                    }
                } else if (notification.data.model === 'push_transacional') {
                    url += this.getUrlTransactional(notification);
                }
            }

            clients.openWindow(url).then((client) => {

                setTimeout(() => { // Wait for new page to finish opening
                    client.postMessage({ type: 'NOTIFICATION_READ', data: notification.data });
                }, 10000);
            });
        }
        getUrlTransactional(notification) {
            switch(notification.data.page) {
                case 'DetalheDoPedido':
                    let compraId = notification.data.compra;
                    return compraId ? '/minha-conta/pedidos/detalhe/' + compraId : '';
                case 'Carrinho':
                    return '/checkout/carrinho';
                case 'ProdutoDetalhe':
                    let produtoId = notification.data.produto;
                    return produtoId ? '/produtos/detalhe/' + produtoId : '';
                case 'Indicacoes':
                case 'IndicacoesDescontos':
                    return '/minha-conta/descontos-de-indicacao';
                default:
                    return '';
            }
        }
