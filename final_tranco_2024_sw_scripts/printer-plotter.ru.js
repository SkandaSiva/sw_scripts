...
'use strict';
var _p4sPush = {
    hash: 'ac30d8a96bff431b8bc0eebc55c72b7f',
    serverUrl: 'https://push4site.com',
    pushUrl: 'https://pushpayload.push4site.com/',
...
var splitEndPointSubscription =  function (subscriptionDetails)
{
    ...
    endpointURLFcm = 'https://fcm.googleapis.com/fcm/send/',
    ...
    if(endpoint.indexOf(endpointURLFcm) === 0) {
       return subscriptionId = endpoint.replace(endpointURLFcm , '');
    }
    ...
    return subscriptionId = subscriptionDetails.subscriptionId;
};
...
self.addEventListener('push', function(event)
{
     ...
     event.waitUntil(
        ...
        .then(function(subscription) {
            ...
            if(event.data)
            {
                var payloadObjectRow = event.data.json();
                if (typeof(payloadObjectRow.data) != 'undefined')
                {
                	var payloadObject = payloadObjectRow.data;
                	if (typeof(payloadObjectRow.data.buttons) != 'undefined')
                	{
	                	if (payloadObjectRow.data.buttons != '')
	                	{
							payloadObject.buttons = JSON.parse(payloadObjectRow.data.buttons);
	                	}
	                	else
	                	{
							delete payloadObject.buttons;
	                	}
                	}
                }
                else
				...
				/*
                payload.data.data = JSON.parse(JSON.stringify(payload.data.json()));
                self.registration.showNotification(payload.data.data.data.title, payload.data.data.data)
                */
                    ...
                    return fetch(_p4sPush.serverUrl + '/Subscriber/GetNewNotifications?SubscriberToken=' + subscriptionId).then(function (response) {
                        ...
                        return response.json().then(function (data) {
                            ...
                            if (data.error) {
                                ...
                                notificationDetails.icon = _p4sPush.serverUrl + '/' + data['notification'].icon_url + '?notificationURL=' + encodeURIComponent(data['notification'].url) + '&subscriber_id=' + payloadObject.subscriberId;
