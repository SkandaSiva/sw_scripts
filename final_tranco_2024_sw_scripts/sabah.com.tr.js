...
messaging.setBackgroundMessageHandler(function (payload) {
    return validateAndShowNotification(payload.data);
});

self.addEventListener('message', function (event) {
    return validateAndShowNotification(event.data.data);
});
...
function checkSegmentValidityAsync(tag) {
    return new Promise(function (resolve, reject) {
        checkSegmentValidity(tag, resolve, reject);
    });
}
    ...
    try {
        getStoredData(function (value) {
            if (value) {
                ...
                var storedStatus = storedStatuses.find(ss => ss.Tag == tag);
