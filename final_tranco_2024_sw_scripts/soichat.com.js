  ...
  ${error.stack}`;
    ...
    var AssetGroup = class {
        ...
        async unhashedResources() {
            const cache = await this.cache;
            return (await cache.keys().catch(() => {})).map((request) => this.adapter.normalizeUrl(request.url)).filter((url) => !this.hashes.has(url));
        }
    ...
    var DataGroup = class {
        ...
        async clearCacheForUrl(url) {
            ...
            await Promise.all([
                cache.delete(this.adapter.newRequest(url, { method: "GET" }), this.config.cacheQueryOptions).catch(() => {}),
                cache.delete(this.adapter.newRequest(url, { method: "HEAD" }), this.config.cacheQueryOptions).catch(() => {}),
                ageTable.delete(url),
            ]);
        }
        async safeFetch(req) {
    ...
    var DebugHandler = class {
        ...
        async handleFetch(req) {
            const [state, versions, idle] = await Promise.all([this.driver.debugState(), this.driver.debugVersions(), this.driver.debugIdleState()]);
    ...
    var Driver = class {
        ...
        constructor(scope2, adapter2, db) {
            ...
            this.scope.addEventListener("message", (event) => {
                event.waitUntil(
                    (async () => {
                        var clients = await this.scope.clients.matchAll({
                            includeUncontrolled: true,
                        });
                        this.screenIsActive = clients && clients.length > 0;
                        if (event.data && event.data.type === "MSG_SCREEN_ACTIVE") {
                            //Process message
                            this.screenIsActive = true;
                            console.log("sw screen is active");
                        }
                        if (event.data && event.data.type === "MSG_SCREEN_INACTIVE") {
                            //Process message
                            this.screenIsActive = false;
                            console.log("sw screen is inactive");
                        }
                    })(),
                );
            });
        ...
        async handlePush(data) {
            ...
            const visible = await this.checkClientIsVisible();

            let title = desc["title"];

            const showAlways = this.isShowAlways(data);

            if (navigator.userAgent.toLowerCase().indexOf("iphone") > -1 || navigator.userAgent.toLowerCase().indexOf("ipad") > -1) {
                console.log("ios notification: " + title + " visible: " + visible + " showAlways: " + showAlways);
            }

            if (!visible || showAlways) {
                await this.scope.registration.showNotification(title, options);
            }
        }
        ...
        isShowAlways(data) {
            ...
            if (data.notification.data && data.notification.data.messageType) {
                const mt = data.notification.data.messageType;
                return (
                    mt === 20 || // video invite
                    mt === 25 // audio invite
                );
        ...
        async checkClientIsVisible() {
            const windowClients = await this.scope.clients.matchAll({
                includeUncontrolled: true,
            });

            let retvalue = false;
            for (var i = 0; i < windowClients.length; i++) {
                if (!retvalue && windowClients[i].visibilityState === "visible") {
                    retvalue = true;
                }
            }
        ...
        async handleClick(notification, action) {
            ...
            const onActionClick =
                (_b = (_a = notification == null ? void 0 : notification.data) == null ? void 0 : _a.onActionClick) == null ? void 0 : _b[notificationAction];
...
(() => {
    var MyServiceWorker = class {
        ...
        async setBadgeCount(data) {
            const badgeCount = data.notification?.data?.badge;
            if (navigator.setAppBadge) {
                if (badgeCount && badgeCount > 0) {
                    await navigator.setAppBadge(badgeCount);
                    console.log(`set badge count to ${badgeCount}`);
                } else {
                    await navigator.clearAppBadge();
                    console.log(`cleared badge count`);
                }
            } else {
                console.log(`badging not supported on this platform`);
            }
        }
    };

    var scope = self;
    new MyServiceWorker(scope);
})();
