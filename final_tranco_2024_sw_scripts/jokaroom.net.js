...
workbox.setConfig( {"debug":false,"modulePathPrefix":"https:\/\/www.jokaroom.net\/wp-content\/plugins\/pwa\/wp-includes\/js\/workbox-v7.0.0\/"} );
workbox.core.setCacheNameDetails( {"prefix":"wp-\/en\/","precache":"precache-front","suffix":"v1"} );
self.skipWaiting();
...
(() => {
	wp.serviceWorker.precaching.precache([{"url":"https:\/\/www.jokaroom.net\/en\/?wp_error_template=offline","revision":"0.8.1;mercury=3.9.7;options=110a919839d47a909907c6e1df95f7d3;nav=a3975ef5999964350e62079184a770e3;deps=ac60a98fc8f51a556eb3b99d55be4013;7f0f92e2178011eb169c69e1b388f1ee"},{"url":"https:\/\/www.jokaroom.net\/en\/?wp_error_template=500","revision":"0.8.1;mercury=3.9.7;options=110a919839d47a909907c6e1df95f7d3;nav=a3975ef5999964350e62079184a770e3;deps=ac60a98fc8f51a556eb3b99d55be4013;f4dc78648272c90eac5253e352b63bd8"}]);
		...
		return fetch(event.request)
					...
					return caches
						.match(
							wp.serviceWorker.precaching.getCacheKeyForURL(
								"https:\/\/www.jokaroom.net\/en\/?wp_error_template=500"
							)
						)
			...
			.catch(() => {
				...
				return caches
					.match(
						wp.serviceWorker.precaching.getCacheKeyForURL(
							"https:\/\/www.jokaroom.net\/en\/?wp_error_template=offline"
						)
					)
...
(() => {
	...
	const errorMessages = {"clientOffline":"It seems you are offline. Please check your internet connection and try again.","serverOffline":"The server appears to be down, or your connection isn't working as expected. Please try again later.","error":"Something prevented the page from being rendered. Please try again.","submissionFailure":"Your submission failed. Please go back and try again."};
	const navigationRouteEntry = {"url":null,"revision":"0.8.1;mercury=3.9.7;options=110a919839d47a909907c6e1df95f7d3;nav=a3975ef5999964350e62079184a770e3;deps=ac60a98fc8f51a556eb3b99d55be4013"};
	...
	const denylist = ["^\\\/wp\\-admin($|\\?|\/)","^[^\\?]*?\\.php($|\\?)","\\?(.*?&)?wp_service_worker=","^[^\\?]*?\\\/wp\\.serviceworker(\\?|$)","^[^\\?]*?\\\/feed\\\/(\\w+\\\/)?$","\\?(.*?&)?wp_customize=","\\?(.*?&)?customize_changeset_uuid=","^\\\/en\\\/wp\\-json\\\/"].map(
...
wp.serviceWorker.routing.registerRoute(
	new wp.serviceWorker.routing.NavigationRoute(
		...
		{
			allowlist: ["^\\\/wp\\-admin($|\\?|\/)","^[^\\?]*?\\.php($|\\?)","\\?(.*?&)?wp_service_worker=","^[^\\?]*?\\\/wp\\.serviceworker(\\?|$)","^[^\\?]*?\\\/feed\\\/(\\w+\\\/)?$","\\?(.*?&)?wp_customize=","\\?(.*?&)?customize_changeset_uuid=","^\\\/en\\\/wp\\-json\\\/"].map(
				(pattern) => new RegExp(pattern)
			),
		}
	)
