...
event.waitUntil(new Promise(resolve=>{if(self.registration.navigationPreload)
{self.registration.navigationPreload[supportPreloading?'enable':'disable']()}
resolve()}),)})
self.addEventListener('message',event=>{const clientId=event.source.id
...
return}
const response=Promise.resolve(event.preloadResponse).then(r=>r||fetch(request))
event.respondWith(response.catch(error=>caches.open(getCacheName()).then(cache=>cache.match(OFFLINE_ROUTE))),)})
self.addEventListener('push',event=>{if(!(self.Notification&&self.Notification.permission==='granted'))
{return}
let data
try
...
return}
const options={body:data.body,dir:data.dir||'ltr',data:data,}
if(data.badge)
...
self.addEventListener('notificationclick',event=>{const notification=event.notification
notification.close()
...
break}}
if(matchingClient)
{return matchingClient.navigate(urlToOpen).then(client=>{if(client)
{client.focus()}
else
...
return}
clients.get(clientId).then(client=>{client.postMessage({type:type,payload:payload,})}).catch(error=>{console.error('An error occurred while sending a message:',error)})}
...
return caches.delete(cacheName).then(()=>caches.open(cacheName)).then(cache=>fetch(CACHE_ROUTE).then(response=>response.json()).then(response=>{const key=response.key||null
const files=response.files||[]
files.push(OFFLINE_ROUTE)
return cache.addAll(files).then(()=>key)})).catch(error=>{console.error('There was an error setting up the cache:',error)})}
function updateCacheKey(clientId,key)
{sendMessage(clientId,'updateCacheKey',{'key':key})}
messageHandlers.updateCache=(clientId,payload)=>{createCache()}