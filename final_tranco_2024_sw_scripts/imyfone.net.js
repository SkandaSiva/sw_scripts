...
var pushData;
var afterCallback;
var callbackUrl = 'https://notification.imyfone.com/subscription/callbacknet';
...
function createRequestUrl(callback, event) {
    if (pushData && pushData.id) {
        var request = callbackUrl+'?id=' + pushData.id;
        if (event) {
            request += '&event=' + event;
        }
...
function clickEventCallback(event) {
    ...
    event.waitUntil(
        createRequestUrl(afterCallback, 'click')
    );
}
function closeEventCallback(){
    createRequestUrl(null, 'close');
}
