...
const CONTENT_METADATA_DETECTED = "fc-content-metadata-detected";
const synchronizeStorefrontContentFromResponse = (response, clients) => {
  const metadataHeader = response.headers.get(
    "x-front-commerce-content-metadata"
  );
  if (!response.ok || !metadataHeader) {
    return;
  }
  try {
    const contentMetadata = JSON.parse(metadataHeader);
    debug("Broadcasting content metadata", contentMetadata);
    clients.matchAll().then((clients2) => {
...
const manifest = {
  v2_compat: { "useApolloClientQueries": true, "useFormsy": true },
  pwa: { "icon": "public/logo.svg", "appName": "autobernard.com", "description": "Un large choix de véhicules neufs et d'occasion Renault, Peugeot, Mercedes, Citroën, Nissan au meilleur prix à Lyon, Grenoble, Bourg en Bresse, Chambery, Seynod et dans tout le réseau Autobernard", "shortName": "autobernard.com", "themeColor": "#01bcf2", "maskableIcon": "public/logo-maskable.svg" }
};
function setupPwa() {
  var _a, _b, _c;
  const manifest$1 = self.__WB_MANIFEST || [];
  precacheAndRoute(manifest$1);
  cleanupOutdatedCaches();
  const strategy = new CacheFirst();
  const urls = Object.values(
    ((_a = manifest.pwa) == null ? void 0 : _a.offline) ?? {}
  );
  self.addEventListener("install", (event) => {
    event.waitUntil(self.skipWaiting());
    const done = urls.map(
      (path) => strategy.handleAll({
        event,
  ...
  if ((_b = manifest.pwa) == null ? void 0 : _b.offline) {
    offlineFallback((_c = manifest.pwa) == null ? void 0 : _c.offline);
  }
}
