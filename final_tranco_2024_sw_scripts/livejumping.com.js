        ...
        handleClick(notification, action) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                yield clients.matchAll().then(matchedClients => {
                    ...
                    let search_url = new URL('/', location).href;
                    if (notification.tag && notification.tag!='') {
                        url = notification.tag;
                        const segments = notification.tag.split('/');
                        if (segments.length>=4) {
                            search_url = segments[0]+'/'+segments[1]+'/'+segments[2]+'/'+segments[3];
                        } else {
                            search_url = notification.tag;
                        }                        
                    }
                    for (let matchClient of matchedClients) {
                        if (matchClient.url.startsWith(search_url)) {
