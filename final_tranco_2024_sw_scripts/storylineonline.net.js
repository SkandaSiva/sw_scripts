const baseVersion = 32
var today = Date.now();
var siteEpoch = Date.parse("27 sep 2021 00:12:00");
var daysSince = Math.round(((today - siteEpoch)/(60*60*24*1000)) % 365);
var version = daysSince + baseVersion + ":";
var theme_path = 'wp-content/themes/storylineonline';

var offlineFundamentals = [
	theme_path + '/assets/all.js',
	theme_path + '/assets/style.css',
...
self.addEventListener("fetch", function(event) {
	...
	if (event.request.headers.has('Content-Type')){
		if (event.request.headers.get('Content-Type').indexOf('text/html') != -1) {
