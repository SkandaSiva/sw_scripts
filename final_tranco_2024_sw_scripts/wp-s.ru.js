...
const putInCache = async (request, response) => {
	...
	if(request.method!='POST' && uri.hostname == self.location.hostname && self.location.pathname!=uri.pathname && [200, 301, 302].indexOf(response.status)>=0){
		const cache = await caches.open(v2);
...
const cardsRegExp = typeof(Response.redirect)=="function" ? new RegExp('https?://((?:[a-z]+\\.)?wp-s.ru)(/(?:wallpapers|tb_images)/.+\\.(?:jpg|png))') : new RegExp('^/fake_url/');
