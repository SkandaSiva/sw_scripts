        ...
        return fetch(url.protocol + "//" + url.host + url.pathname + ".webp" + url.search, init)
          ...
          .then(function(resp) {
            ...
            if (resp.status == 404) {
              ...
              ;(url.protocol + "//" + url.host + url.pathname).match(matchWebpApi) && fetch(
...
if (!workbox) {
  ...
  // )
  workbox.routing.registerRoute(
    /^https?:\/\/[^/]+\/(?:res\/[\da-z]+\/[\da-f]{24}[^/]*|[^/]+\/res\/[\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12}(?:\/[^/]+)?)\.webp$/,
    workbox.strategies.cacheFirst({
      cacheName: "webp-images",
      plugins: [
        new workbox.cacheableResponse.Plugin({statuses: [0, 200]}),
        new workbox.expiration.Plugin({maxAgeSeconds: 60 * 60 * 24 * 365, maxEntries: 40}),
      ],
    }),
  )
