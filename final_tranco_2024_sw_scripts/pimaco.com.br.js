function shouldCache(response) {
  return response.status !== 0 && !(response.status >= 400 && response.status < 500) || response.type === "opaque" || response.type === "opaqueredirect";
}
  ...
  if (url.pathname.startsWith("/favicons/") || url.pathname.startsWith("/fonts/") || url.pathname.startsWith("/build/")) {
    event.respondWith(
      ...
      caches.open("assets").then(async (cache) => {
        ...
        if (shouldCache(fetchResponse) && !event.request.url.startsWith('extension') && !event.request.url.startsWith('chrome-extension')) {
