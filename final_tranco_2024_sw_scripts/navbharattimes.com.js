importScripts("wb-manifest.60144109bbe17651b1eb050c29e4f6f4.js", "https://storage.googleapis.com/workbox-cdn/releases/3.6.3/workbox-sw.js");
...
if (workbox) {
  ...
  const HIGH_CACHE_FILES = [
    ...
    "https://securepubads.g.doubleclick.net/tag/js/gpt.js?network-code=<7176>",
    ...
    "https://cdn.taboola.com/libtrc/timesinternetlimited-navbharattimes/loader.js",
    "https://timesofindia.indiatimes.com/lgads_v2/minify-1,channel-nbt.cms",
  ];

  workbox.routing.registerRoute(
    ({ url }) => HIGH_CACHE_FILES.some(ele => ele.includes(url.pathname)),
  ...
  const fetchMsid = url =>
    url
      .split("/")
      .filter(item => item.indexOf(".cms") > -1)[0]
      .split(".")[0];
        ...
        idb.getData("userResetTime", data).then(resp1 => {
          ...
          idb.getData(_key, data).then(resp => {
            ...
            if (userSecMap.size >= 10) {
              userSecMap = new Map([...userSecMap].sort((a, b) => b[1] - a[1]));
